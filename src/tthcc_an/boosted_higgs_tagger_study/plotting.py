...
try:
    ...
    def gaussian_filter(
        ...
        **_kwargs: Any,
        ...
        """Keep plots usable when the optional display-smoothing dependency is absent."""
...
def _hcc_scan_markers(
    ...
    markers = [
        {
            "label": "Current loose",
            "x": current_references["loose"]["x_cut"],
            "y": current_references["loose"]["y_cut"],
            "marker": "o",
            "color": "#f5c542",
        },
        {
            "label": "Current tight",
            "x": current_references["tight"]["x_cut"],
            "y": current_references["tight"]["y_cut"],
            "marker": "s",
            "color": "#f28e2b",
        },
    ]
    ...
    global_point = next(
        (row for row in recommendations if row["label"] == "global_max"),
        None,
    )
        ...
        markers.append(
            {
                ...
                "x": global_point["x_cut"],
                "y": global_point["y_cut"],
                "marker": "*",
                "color": "#d62728",
            }
        )
    constrained = [
        row for row in recommendations if row["label"].startswith("background_eff_le_")
    ]
    if constrained:
        indices = sorted({0, len(constrained) // 2, len(constrained) - 1})
        for index in indices:
            row = constrained[index]
            constraint_label = row["label"].replace("background_eff_le_", "B eff <= ")
            markers.append(
                {
...
def _plot_hcc_scan_heatmap(
    ...
    colorbar.set_label(colorbar_label, fontsize=plot_style.label_size)
    colorbar.ax.tick_params(labelsize=plot_style.tick_size)
    for marker in _hcc_scan_markers(result, current_references):
        ax.scatter(
            marker["x"],
            marker["y"],
            marker=marker["marker"],
            s=105 if marker["marker"] == "*" else 58,
            c=marker["color"],
            edgecolors="white",
            linewidths=0.8,
            label=marker["label"],
    ...
    ax.set_xlim(float(result["x_cuts"][0]), float(result["x_cuts"][-1]))
    ax.set_ylim(float(result["y_cuts"][0]), float(result["y_cuts"][-1]))
    ax.set_xlabel("gParT3 Higgs vs QCD lower cut", fontsize=plot_style.label_size)
    ax.set_ylabel("gParT3 Xbb vs Xcc upper cut", fontsize=plot_style.label_size)
    ax.set_title(title, fontsize=plot_style.title_size, pad=10)
    ax.tick_params(axis="both", labelsize=plot_style.tick_size)
    ax.legend(
        loc="lower left",
        fontsize=max(plot_style.legend_size - 1.0, 7.0),
        frameon=True,
        ncol=2,
    )
    _cms_label(ax, plot_style)
    _save_plot(fig, outpath, plot_style, left=0.12, right=0.96, bottom=0.12, top=0.88)


def plot_hcc_wp_scan_heatmaps(
    result: dict[str, Any],
    outdir: Path,
    plot_style: PlotStyle,
    current_references: dict[str, dict[str, Any]],
    ...
    if result["level"] == "jet":
        specifications = [
            ("significance_heatmap.png", result["significance"], "Hcc jet WP significance", r"$S/\sqrt{S+B}$"),
            ("relative_significance_heatmap.png", result["relative_significance"], "Relative Hcc jet significance", r"$Z/Z_{baseline}$"),
            ("hcc_pure_efficiency_heatmap.png", result["signal_efficiency"], "hcc_pure efficiency", "Efficiency"),
            ("background_efficiency_heatmap.png", result["background_efficiency"], "Total background efficiency", "Efficiency"),
            ("signal_over_background_heatmap.png", result["signal_over_background"], "Hcc jet signal-to-background ratio", "S/B"),
            ("hbb_pure_leakage_heatmap.png", result["truth_efficiencies"]["hbb_pure"], "hbb_pure leakage into Hcc", "Efficiency"),
            ("other_efficiency_heatmap.png", result["truth_efficiencies"]["other"], "Other-jet efficiency", "Efficiency"),
        ]
        for process, filename, title in [
            ("qcd", "qcd_efficiency_heatmap.png", "QCD process efficiency"),
            ("ttbar", "ttbar_efficiency_heatmap.png", "ttbar process efficiency"),
        ]:
            if process in result["process_efficiencies"]:
                specifications.append((filename, result["process_efficiencies"][process], title, "Efficiency"))
    else:
        specifications = [
            ("significance_heatmap.png", result["significance"], "Hcc event-acceptance significance", r"$S/\sqrt{S+B}$"),
            ("tthcc_efficiency_heatmap.png", result["signal_efficiency"], "ttHcc event efficiency", "Efficiency"),
            ("background_efficiency_heatmap.png", result["background_efficiency"], "Event-level background efficiency", "Efficiency"),
        ]
    for filename, values, title, colorbar_label in specifications:
        _plot_hcc_scan_heatmap(
            values,
            result,
            outdir / filename,
            title,
